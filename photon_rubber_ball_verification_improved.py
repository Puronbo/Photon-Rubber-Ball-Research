...
if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding="utf-8")  # print Greek/math glyphs on stock Windows (cp1252) consoles
...
# Supplemental constants shared via star-import by the *_ANALOGY / *_COMPARISON scripts
HBAR = 1.054571817e-34      # Reduced Planck constant (J·s)
G = 6.67430e-11             # Gravitational constant (m^3 kg^-1 s^-2)
C = C0                      # Speed of light (m/s) - alias of C0
BOLTZMANN_CONSTANT = KB     # Boltzmann constant (J/K) - alias of KB
LAMBDA_PHOTON = 550e-9      # Verification-laser wavelength (m)
...
# Test parameters for axes (configurable)
AXIS2_MODULUS_VALUES = (10e6, 50e6, 100e6)  # Pa
AXIS3_TANDELTA_VALUES = (0.05, 0.1, 0.2)    # dimensionless
AXIS4_BETA_VALUES = (0.01, 0.04, 0.1)       # v/c
AXIS9_TEST_PARAMS = {
    'pmean_expected': 4.11e6,   # Pa
    'ppeak_expected': 6.17e6,   # Pa
    'f0_expected': 3.75e7,      # Hz
    're_water_expected': 0.605, # dimensionless
    'v_th_expected': 11.4e-3,   # m/s
    'dT_expected': 0.661        # K
}
...
def compute_indentation_pressure(E_val: float, nu_val: float = NU) -> Tuple[float, float]:
    ...
    """
    if E_val <= 0 or abs(nu_val) >= 1:
        raise ValueError(f"compute_indentation_pressure: need E>0 and -1<nu<1, got E={E_val}, nu={nu_val}")
...
def sim_hunt_crossley(tand: float, dt: float = 5e-12, v0: float = 1.0) -> Tuple[float, float]:
    ...
    """
    if v0 <= 0:
        raise ValueError(f"sim_hunt_crossley: need v0>0, got v0={v0}")
...
def jkr_contact_radius(load: float, work_adhesion: float = 0.1,
    ...
    """
    if radius <= 0 or modulus <= 0 or work_adhesion < 0:
        raise ValueError(f"jkr_contact_radius: need radius>0, modulus>0, w>=0, got r={radius}, E={modulus}, w={work_adhesion}")
...
def jkr_indentation(radius_contact: float, work_adhesion: float = 0.1,
    ...
    Returns:
        Indentation depth (m)
    """
    if radius_contact < 0 or work_adhesion < 0 or radius <= 0 or modulus <= 0:
        raise ValueError(f"jkr_indentation: need a>=0, r>0, E>0, w>=0, got a={radius_contact}, r={radius}, E={modulus}, w={work_adhesion}")
...
def jkr_load(radius_contact: float, work_adhesion: float = 0.1,
    ...
    """
    if radius_contact < 0 or work_adhesion < 0 or radius <= 0 or modulus <= 0:
        raise ValueError(f"jkr_load: need a>=0, r>0, E>0, w>=0, got a={radius_contact}, r={radius}, E={modulus}, w={work_adhesion}")
...
def jkr_work_of_separation_integrand(radius_contact: float,
    ...
    """
    if radius_contact <= 0 or work_adhesion < 0 or radius <= 0 or modulus <= 0:
        raise ValueError(f"jkr_work_of_separation_integrand: need a>0, r>0, E>0, w>=0, got a={radius_contact}, r={radius}, E={modulus}, w={work_adhesion}")
...
def mie_spherical_bessel_jn(z: float, N: int) -> Tuple[complex, ...]:
    """
    ...
    Returns:
        Tuple of j_n(z) values for n=0..N
    """
    v = [0j] * (N + 2)
    v[N] = 1+0j
    v[N+1] = 0+0j
    ...
    scale = math.sqrt(s)
    return tuple(v[n]/scale for n in range(N+1))

@lru_cache(maxsize=32)
def mie_spherical_bessel_yn(z: float, N: int) -> Tuple[complex, ...]:
    """
    ...
    Returns:
        Tuple of y_n(z) values for n=0..N
    """
    y = [0j] * (N + 1)
    y[0] = -math.cos(z)/z
    if N >= 1:
...
def mie_q(mk: complex, x: float, N: Optional[int] = None) -> Tuple[float, float]:
    ...
    for n in range(1, N+1):
        ...
        # Extinction must sum Re(a_n + b_n); omitting b_n breaks the energy
        # balance Q_ext = Q_sca + Q_abs for lossless (real-index) scatterers.
...
def verify_axis2_modulus_sensitivity() -> bool:
    ...
    for Ee in AXIS2_MODULUS_VALUES:
        Ese = Ee/(1-NU**2)
    ...
    d10 = ((15/16)*M*V**2/((AXIS2_MODULUS_VALUES[0]/(1-NU**2))*math.sqrt(R)))**(2/5)
    d100 = ((15/16)*M*V**2/((AXIS2_MODULUS_VALUES[2]/(1-NU**2))*math.sqrt(R)))**(2/5)
...
def verify_axis3_viscoelastic_restitution() -> bool:
    ...
    for tand in AXIS3_TANDELTA_VALUES:
        zeta = tand/2
        e_lin = math.exp(-math.pi*zeta/math.sqrt(1-zeta*zeta))
        e_sim = simulate_linear_oscillator(zeta)
        passed = check_condition(
    ...
    es_hc = []
    for tand in AXIS3_TANDELTA_VALUES:
        v_out, dfrac = sim_hunt_crossley(tand)
        e_hc = v_out
        es_hc.append(e_hc)
        passed = check_condition(
...
def verify_axis8_mie_scattering() -> bool:
    ...
    Q2e, Q2s = mie_q(mk, x)
    Q3e, Q3s = mie_q(mk, x, N=int(x+4*abs(x)**(1/3))+8+8)
    passed &= check_condition(
...
def verify_axis9_consolidated_numbers() -> bool:
    ...
    passed = check_condition(
        ...
        abs(pmean-AXIS9_TEST_PARAMS['pmean_expected'])/AXIS9_TEST_PARAMS['pmean_expected'] < 0.01,
        f"{pmean/1e6:.3f} MPa"
    )
    all_passed &= passed
    ...
    passed &= check_condition(
        ...
        abs(ppeak-AXIS9_TEST_PARAMS['ppeak_expected'])/AXIS9_TEST_PARAMS['ppeak_expected'] < 0.01,
        f"{ppeak/1e6:.2f} MPa"
    )
    all_passed &= passed
    ...
    passed = check_condition(
        ...
        abs(f0-AXIS9_TEST_PARAMS['f0_expected'])/AXIS9_TEST_PARAMS['f0_expected'] < 0.01,
        f"{f0/1e9:.4f} GHz"
    )
    all_passed &= passed
    ...
    passed = check_condition(
        ...
        abs(Re_w-AXIS9_TEST_PARAMS['re_water_expected'])/AXIS9_TEST_PARAMS['re_water_expected'] < 0.01,
        f"Re={Re_w:.3f}"
    )
    all_passed &= passed
    ...
    passed = check_condition(
        ...
        abs(v_th-AXIS9_TEST_PARAMS['v_th_expected'])/AXIS9_TEST_PARAMS['v_th_expected'] < 0.02,
        f"{v_th*1e3:.2f} mm/s"
    )
    all_passed &= passed
    ...
    passed = check_condition(
        ...
        abs(dT-AXIS9_TEST_PARAMS['dT_expected'])/AXIS9_TEST_PARAMS['dT_expected'] < 0.01,
        f"dT={dT:.3f}K (conservative {dT_low:.3f}K)"
    )
    all_passed &= passed

    # Note: Removed premature print of PASS/FAIL here; will be printed in main
    return all_passed
...
def main() -> None:
    ...
    print(f"\n=== FINAL RESULT: {pass_count} passed, {fail_count} failed ===")
