...
\subsection{Probe-Particle Type Dimension}\label{sec:probetype}
The model uses a massless photon (550 nm, $E=2.25$ eV) as its probe. For a probe of rest-mass energy $mc^2$ incident at the same quantum energy $E$, the absorbed momentum flux is $F=(p/E)P$ with $p/E=v/c^2$ and $v/c=\sqrt{1-(mc^2/E)^2}$, so $F=(v/c)(P/c)$ saturates at the massless ceiling $F=P/c=3.34$ pN at 1 mW. Massless and low-mass probes are observationally identical while $(mc^2/E)^2\ll10^{-6}$ (any real photon: $(mc^2/E)^2\approx2\times10^{-37}$); deviations reach percent level only for $mc^2$ within $\sim$10$\times$ of $E=2.25$ eV, where a 1 eV rest-mass boson would give $v/c=0.90$, a 10.4\% force loss, and a 0.21 fs transit-delay across the 550 nm diameter ball (Table~\ref{tab:probetype}). Other Standard-Model bosons cannot be used as beams in this model's scattering language at all: gluons are confined ($\sim$1 fm), $W^\pm$/$Z^0$ self-decay in $\sim$$10^{-25}$ s ($c\tau\sim$0.1 fm), and the Higgs requires $\geq$125 GeV production. A slow, low-mass (hidden/dark-photon) boson would exhibit coherent whole-ball absorption instead of scattering --- a proposed direction, not asserted numerically here.

\begin{table}[h!]
\centering
\caption{Massless vs low-mass probe at $E=2.25$ eV and 1 mW ($F_{max}=P/c=3.34$ pN). Computed by \texttt{boson\_scaling\_probe.py} (19-script battery).}
\label{tab:probetype}
\begin{tabular}{lcccl}
\toprule
$mc^2$ [eV] & $(mc^2/E)^2$ & $v/c$ & $F$ [pN] & transit delay [fs] \\
\midrule
0 (massless)      & 0                  & 1.000 & 3.336 & 0 \\
$10^{-18}$ (bound) & $2.0\times10^{-37}$ & 1.000 & 3.336 & 0 \\
$10^{-3}$         & $2.0\times10^{-7}$  & 1.000 & 3.336 & $<10^{-6}$ \\
$10^{-1}$         & $2.0\times10^{-3}$  & 0.999 & 3.332 & 0.002 \\
1                 & 0.197               & 0.896 & 2.990 & 0.212 \\
$\geq E$          & $\geq 1$            & --    & --     & no ultra-relativistic beam \\
\bottomrule
\end{tabular}
\end{table}
